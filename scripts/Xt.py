...
model_name = [["ddrobot::front_left","ddrobot::front_right"],["ddrobot::back_left","ddrobot::back_right"]]
...
def main():
    ...
    while not rospy.is_shutdown():
        for i in range(0,2):
            for j in range(0,2):
                #wheel_Xt[2*i+j] = float(Xt_map[int(wheel_position_x[i][j]*10) % 1000][int(wheel_position_y[i][j]*10) % 1000])
		wheel_Xt[2*i+j] = 1
        msg.data = wheel_Xt
        pub.publish(msg)
        rate.sleep()
