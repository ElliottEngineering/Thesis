...
const float prismaticJointLimit = 0.29;
...
private:
  ...
  float prismaticPosition[notN - 1];
...
public:
  ...
  float getPrismaticPosition(int i){
	  return prismaticPosition[i];
  }
	...
	void joint_states_Callback(const sensor_msgs::JointState::ConstPtr& msg){
		...
		if(!first){
			...
			for(size_t i=0; i<(notN-1); ++i){
				prismaticPosition[i] = msg->position[prismaticJointOrder[i]];
			...
			for(size_t h=0; h<names.size(); ++h){
				...
				for(size_t i=0; i<(notN-1); ++i){
						if(names[h].compare(prismatic_name[i]) == 0){
							...
							ROS_INFO_STREAM("prismaticJoint " << i << " found");
						}
					for(size_t j=0; j<2; ++j){
  ...
  void heaveHoCallback(const ros::TimerEvent&){
	...
	if(driveType == "inch1" && heaveHo >= 2.5){
		heaveHo = 0;
		leftRight = -leftRight;
	}else if((driveType == "inch2"|| driveType == "two-to-one" || driveType == "two-to-one-2") && heaveHo >= 3.75){
		heaveHo = 0;
		leftRight = -leftRight;
	}
	...
	while(ros::ok()){
		...
		if(v1.checkRunController()){ //this is the navigation controller that sets desired wheel speeds
			...
			if (v1.getDriveType().compare("normal") == 0) {
				...
				for(size_t i=0; i<notN; ++i){
					...
					if(i == 0){ //if lead segment
						...
						*/
						...
						/*
						// this is for driving the vehicle in a circle
				...
				float connectorForceP1 = 200;
				float connectorForceP2 = 500;
				float connectorForceD2 = 0;
				...
				for(int i=0; i<notN-1; ++i){
					...
					if(i % 3 == 0){//every third connector (would include first)
						if(phase == 0){ 
							...
							static float oldError2 = error2;
							
							if(v1.getPrismaticPosition(i) < -prismaticJointLimit){
								prismaticSetForce[i] = 0;
								//ROS_INFO_STREAM("limit reached");
							}else{
								prismaticSetForce[i] = -connectorForceP1*error1 - connectorForceP2*error2*M - connectorForceD2*(error2 - oldError2)/controllerLoopRate;
								oldError2 = error2;
							}
							...
							if(v1.getPrismaticPosition(i) > prismaticJointLimit){
								...
								prismaticSetForce[i] = connectorForceP1*error1 + connectorForceP2*error2 + connectorForceD2*(error2 - oldError2)/controllerLoopRate;
								oldError2 = error2;
							}
			...
			else if (v1.getDriveType().compare("two-to-one-2") == 0) {
				...
				float lowerLimit = -noLoadSpeed;
				float P3 = 10;
				float linearDriveSpeed = 0.8;
				float driveSpeed = linearDriveSpeed / wheelRadius;
				float idleSpeed = 0.0 / wheelRadius;
				float anchorSpeed = 0.00 / wheelRadius;
				float heaveHo_L = v1.get_heaveHo();	
				int phase = 0;
				...
				float constraint = forceLimit / Ki;
				
				const float anchorSetSpeed = -0.05;
				...
				for(int i=0; i<notN-1; ++i){
					...
					if(i % 3 == 0){//every third connector (would include first)
						if(phase == 0){ 
							float error1 = anchorSetSpeed - v1.getBodyLonRate(i+1);	//stationary segment	
							
							static float errorInt = 0;
							errorInt = constrain(errorInt + error1,-constraint,constraint);
							
							if(v1.getPrismaticPosition(i) < -prismaticJointLimit){
								prismaticSetForce[i] = 0;
								//ROS_INFO_STREAM("limit reached");
							}else{
								prismaticSetForce[i] = (Kp*error1 + Ki*errorInt);
							}
							
						}else if(phase == 2){							
							float error1 = anchorSetSpeed - v1.getBodyLonRate(i);	//stationary segment
					...
					prismaticSetForce[i] = constrain(prismaticSetForce[i],-forceLimit,forceLimit);
					//connectorForce[i] = connectorForce[i] + 1 - fabs(v1.getBodyLonRate(i)) - fabs(v1.getBodyLonRate(i+1));
					msg_prismaticSetForce.data[i] = prismaticSetForce[i];
				}
