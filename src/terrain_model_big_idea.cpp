...
int main(int argc, char **argv){
	...
	std::string model_name[2][2] = {{"winch::wheel_0_0","winch::wheel_0_1"},{"winch::wheel_1_0","winch::wheel_1_1"}};
  std::string joint_name[2][2] = {{"joint_wheel_0_0","joint_wheel_0_1"},{"joint_wheel_1_0","joint_wheel_1_1"}};
  ...
  ros::Subscriber sub5 = terrain_model_node.subscribe("/winch/joint_states", 10, joint_states_Callback);
  ...
  ROS_INFO_STREAM("#########################################################");
    ...
    for(int i=0; i<2; ++i){
      for(int j=0; j<2; ++j){
        double COEFF = coeffTerrainResistanceTorque*Xt[i][j];
