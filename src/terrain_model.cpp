...
double normalForce[2][2];
double wheelAngularSpeed[2][2];
double wheelLonSpeed[2][2];
double wheelLatSpeed[2][2];
double wheelYaw[2][2];
double wheelYawRate[2][2];
...
const double yamlRate = 200; // must be same as in YAML
...
void bumper_front_right_Callback(const gazebo_msgs::ContactsState::ConstPtr& msg)
{
  normalForce[0][1] = force_from_wheel(msg);
}

void bumper_front_left_Callback(const gazebo_msgs::ContactsState::ConstPtr& msg)
{
  normalForce[0][0] = force_from_wheel(msg);
}

void bumper_back_left_Callback(const gazebo_msgs::ContactsState::ConstPtr& msg)
{
  normalForce[1][0] = force_from_wheel(msg);
}

void bumper_back_right_Callback(const gazebo_msgs::ContactsState::ConstPtr& msg)
{
...
void link_states_Callback(const gazebo_msgs::LinkStates::ConstPtr& msg){
    const std::vector<std::string> &names = msg->name;
    for(size_t i=0; i<names.size(); ++i){
      if(names[i].compare("ddrobot::back_left") == 0){
      ...
      else if (names[i].compare("ddrobot::back_right") == 0){
      ...
      else if (names[i].compare("ddrobot::front_left") == 0){
      ...
      else if (names[i].compare("ddrobot::front_right") == 0){
...
int main(int argc, char **argv){
	...
	std::string model_name[2][2] = {{"ddrobot::front_left","ddrobot::front_right"},{"ddrobot::back_left","ddrobot::back_right"}};
  ...
  ros::Subscriber sub1 = terrain_model_node.subscribe("/bumper_front_left", 10, bumper_front_left_Callback);
  ros::Subscriber sub2 = terrain_model_node.subscribe("/bumper_front_right", 10, bumper_front_right_Callback);
  ros::Subscriber sub3 = terrain_model_node.subscribe("/bumper_back_left", 10, bumper_back_left_Callback);
  ros::Subscriber sub4 = terrain_model_node.subscribe("/bumper_back_right", 10, bumper_back_right_Callback);
    ...
    */
    ...
    for(int i=0; i<2; ++i){
      for(int j=0; j<2; ++j){
        ...
        terrainResistanceTorque[i][j] = constrain(terrainResistanceTorque[i][j],-100,100)*normalForce[i][j];
      }
    }
    ...
    for(int i=0; i<2; ++i){
      for(int j=0; j<2; ++j){
        double COEFF = coeffTerrainResistanceForce/Xt[i][j];
    ...
    */
    ...
    for(int i=0; i<2; ++i){
      for(int j=0; j<2; ++j){
        ...
        yawResistanceTorque[i][j] = -COEFF*signSpeed(wheelYawRate[i][j])*normalForce[i][j];
      }
    }
    */
    ...
    //  for(int j=0; j<2; ++j){
        ...
        //ROS_INFO_STREAM("motorTorque is " << motorTorque[i][j]);
        static double meanTorque = 0;
        meanTorque = meanTorque*0.99 + motorTorque[i][j]*0.01;
        ROS_INFO_STREAM("mean torque " << meanTorque);
    ...
    */
    ...
    for(int i=0; i<2; ++i){
      for(int j=0; j<2; ++j){
        ...
        apply_wrench_req.wrench.force.x = terrainResistanceForce[i][j]*cos(wheelYaw[i][j]);
        apply_wrench_req.wrench.force.y = terrainResistanceForce[i][j]*sin(wheelYaw[i][j]);
