...
public:
 ...
 bool checkPrismaticJointsPresent(){
	 return prismaticJointsPresent;
 }
...
int main(int argc, char **argv){
  ...
  while(ros::ok()){
    ...
    if(v1.checkRunSpeedControl()){ //This is the closed loop speed controller
		...
		for(int i=0; i<segmentNumber; ++i){
			for(int j=0; j<2; ++j){
				for(int k=0; k<2; ++k){
					...
					speedErrorIntegral[i][j][k] = constrain(speedErrorIntegral[i][j][k] + (speedDiff/speedControllerLoopRate),-1.2,1.2);
					...
					//newSetPoint = 1.0*motorSetSpeed_l/noLoadSpeed; //this line makes speed controller run open loop
	  ...
	  if(v1.checkPrismaticJointsPresent()){
