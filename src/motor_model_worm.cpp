...
int main(int argc, char **argv){
  ...
  ros::Subscriber sub5 = motor_model_node.subscribe("/worm/joint_states", 10, joint_states_Callback);
    ...
    for(int i=0; i<2; i++){
      for(int j=0; j<2; j++){
        ...
        if(motorSetPoint[i*2 + j] == 2){ //if the value is two, let the motor freewheel
          ...
          msg.data[i*2 + j] = min(slope*(motorSpeed - motorSetPoint[i*2 + j]*noLoadSpeed),110);
        }
